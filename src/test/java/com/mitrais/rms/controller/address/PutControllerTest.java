...
@DataJpaTest
@ComponentScan(basePackageClasses = {PutController.class, AddressService.class})
@RunWith(SpringRunner.class)
public class PutControllerTest
{
	@Autowired
	@Qualifier("addressPutController")
	...
	{
		String body = "{\"address\": \"Jln. Surya Sumantri\"}";
		...
		when().
			patch("/employees/{employeeId}/addresses/{addressId}",
				employeeId, addressId).
		then().
			statusCode(200).
			body("id", equalTo("/employees/" + employeeId + "/addresses/" + addressId)).
			body("activeInd", equalTo(true)).
			body("address", not("hello world")).
			body("address", equalTo("Jln. Surya Sumantri"));
